...
<div id="wrapper">
     <!-- Navigation -->
      ...
      <div class="col_1">
		   <div class="col-md-12 span_6">
                 <div class="panel-heading">
                  <h1>
                    <span class="label btn_6 label-success">Update Ready Status</span>  
                    </h1>
                </div>
                <%
		 ...
		 <div class="tab-pane active" id="horizontal-form">
                    <form class="form-horizontal" name="SendToDyer" action="UpdateStatusDyerDetail.jsp">
      ...
      float QtyOrdered=0,qtySent=0,qtyReady=0,qtyRcvd =0,qtySentToInsp=0,qtyToBeSent=0,qtyShrink=0,qtyShrinkMtrs=0,qtyRj=0;
...
<%
  ...
  " fab.NAME FABRIC_NAME,var.ID VAR_ID,var.NAME VAR_NAME,d.NAME DYER_NAME,p.QTY, pd.QTY_SENT,pd.QTY_RCVD,ifnull(pd.QTY_SHRINK_PRSNT,0) QTY_SHRINK_PRSNT,ifnull(pd.QTY_RJ,0) QTY_RJ,"+
  " DATE_FORMAT(M_SENT_DATE,'%d-%b-%Y') M_SENT_DATE,DATE_FORMAT(M_RCVD_DATE,'%d-%b-%Y') M_RCVD_DATE,"+
  " SUPP_REMARKS, plain.NAME plain_code , QTY_SENT_TO_INSP,ifnull(QTY_SHRINK,0)QTY_SHRINK,ifnull(QTY_READY,0)QTY_READY,(QTY_RCVD-QTY_SENT_TO_INSP) QTY_TO_BE_SENT"+
  ...
  " and pd.STATUS='R' and pd.TYPE_CHECK='Plain' and pd.ALL_READY NOT IN ('Y') order by pd.M_SENT_DATE ASC";
   //out.println(dtlSql1);
		  ...
		  <div class="bs-example1" id="contextual-table" style='overflow-x:scroll;overflow-y:hidden;width:1150x;'>
		      ...
		      <thead>
		        <tr>
		          ...
		          <th>Fabric & Variety</th>
                          ...
                          <th>Dyer</th>
                          <th>Plain Code</th>
                          <th>Qty. <br>ordered As per PO<br>(in Meters)</th>
                          <th>Qty. <br> received at Dyer (in Meters)(a)</th>
                          <th>Material Received Date</th>
                          <th>Estimated Shrink Qty. <br>(in %)</th>
                          <th>Actual Shrink Qty. <br>(in meters) (b)</th>
                          <th>Qty. <br> already sent for inspection (c)</th>
                           <th>Qty. <br> Rejected (r)</th>
                           <th>Total Qty. Ready</th>
                          <th>Qty. to be sent (d=a-b-c)</th>
		      ...
		      <tbody>
		          ...
		          <td align=center><%=QtyOrdered%></td>
                          <td align=center><%=qtyRcvd%></td>
                          <td align=center><%=rcvdDate%></td>
                           <td align=center><%=qtyShrink%></td> 
                          <td align=center><%=qtyShrinkMtrs%></td> 
                          <td align=center><%=qtySentToInsp%></td>
                           <td align=center><%=qtyRj%></td> 
                            <td align=center><%=qtyReady%></td> 
                          <td align=center><%=qtyToBeSent%></td>  
                          <td align=left><a href="ClosePurchase.jsp?dpId=<%=dpId%>&report=dyer&type=ready">Close</a></td>
