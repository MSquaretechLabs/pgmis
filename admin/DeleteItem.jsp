...
{   
 ...
 }else if (type.equalsIgnoreCase("soDetails"))
 {
 String customer= request.getParameter("customer");
 String style= request.getParameter("style");
 ...
 backURL="/pgmis/sales/SODetails.jsp?display=display&customer="+customer+"&so="+so+"&style="+style+"&styleType="+styleType+"&otherCustomer="+otherCustomer+"";
 }
 
// out.println(display);
String deleteSql="",insertLog="";
int x=0;
int y=0;
     try{
    
    
     if (type.equalsIgnoreCase("soDetails")) // delete from combo details
     {
     deleteSql="DELETE FROM t_so_combo_details "+
     " WHERE SO_DTL_ID="+id+"";
     //out.println(sqlUpdate);
     x=st.executeUpdate(deleteSql);
    // message= "Selected Record has been deleted successfully. Please Click Back to go back to previous page.";
     
     insertLog= " INSERT INTO t_delete_log (TRANS_DATE,QUERY) VALUES (NOW(),'"+deleteSql+"')";
     y=st.executeUpdate(insertLog);
